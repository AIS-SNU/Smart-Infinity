...
public:
	...
	// OpenCL related vaiables
	std::thread thr;
	// OpenCL related function
	void Step_fpga_adagrad( 
	...
	inline void IncrementStep_(size_t step)
    {
        _step++;
        if (_step != step) { _step = step; }
    }
	inline void IncrementStep_(size_t step, float beta1)
    {
        _step++;
        if (_step != step) { _step = step; }
		_betta1 = beta1;
