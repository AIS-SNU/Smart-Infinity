...
class DeepSpeedZeroOptimizer_Stage3(ZeROOptimizer):
    ...
    def __init__(self,
        ...
        print("# of fp16 subgroups: ", num_fp16_subgroups )
    ...
    def _setup_for_real_optimizer(self):
        ...
        #print('Dist[', dist.get_rank(),'] comes here (1)')
        self._create_next_swappable_fp32_groups()

        see_memory_usage("Before initializing optimizer states", force=True)

        #print('Dist[', dist.get_rank(),'] comes here (2)')

        #if dist.get_rank() == 0:
        self.initialize_optimizer_states()
        #print('Dist[', dist.get_rank(),'] comes here (3)')
        see_memory_usage("After initializing optimizer states", force=True)
        #print('Dist[', dist.get_rank(),'] comes here (4)')
    ...
    def _configure_tensor_swapping(self, offload_optimizer_config, aio_config):
        ...
        self.nvme_swap_folder = offload_optimizer_config.nvme_path
        ...
        self.optimizer_swapper = swapper_type(swap_config=offload_optimizer_config,
                                              ...
                                              base_folder=self.nvme_swap_folder,
    ...
    def _multiple_optimizer_step_with_fpga(self, sub_group_id, sub_group_chunk_size, combined_unscale, largest_numel):
        for s_id in range(sub_group_id, sub_group_id + sub_group_chunk_size ):
            #print("For subgroup: ", s_id) 
            param_group_id = self.sub_group_to_group_id[s_id]
            fp32_param = self.fp32_partitioned_groups_flat[s_id]
            #self.optimizer.param_groups[param_group_id]['params'] = [fp32_param]
        
            fp16_param = self.fp16_partitioned_groups_flat[s_id]
            self.optimizer.param_groups[param_group_id]['params'] = [(fp16_param, fp32_param)]

            target_device_id = s_id % self.num_ssds
            
            #print("Start C++ codes here")
            self.optimizer.step_with_fpga( target_device_id, self.optimizer_swapper, combined_unscale, largest_numel, self.comp_ratio )
            
            self.optimizer.param_groups[param_group_id]['params'] = []
        
    
    def _optimizer_step_with_fpga(self, sub_group_id, combined_unscale):
        param_group_id = self.sub_group_to_group_id[sub_group_id]
        fp32_param = self.fp32_partitioned_groups_flat[sub_group_id]
        self.optimizer.param_groups[param_group_id]['params'] = [fp32_param]
        
        target_device_id = sub_group_id % self.num_ssds
        self.optimizer.step_with_fpga(sub_group_id, fp32_param, self.optimizer_swapper, combined_unscale, device_id = target_device_id )
    ...
    def initialize_optimizer_states(self):
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            if swappable_optimizer_subgroup:
                self._optimizer_states_and_gradient_swap_in(i, timer_names, use_fpga = False)
            ...
            if swappable_optimizer_subgroup:
                self._optimizer_states_and_gradient_swap_out(i, timer_names, use_fpga = False)
    ...
    def partition_grads(self, params_to_release: List[Parameter], grad_partitions: List[Tensor]) -> None:
        ...
        if self.offload_optimizer and self.swap_optimizer:
            #print("subgroup?: ", offload_fp32_gradients.keys())
            for i in offload_fp32_gradients.keys():
                
                #print("length for ", i, " : ", [ len(t) for t in offload_fp32_gradients[i]])
                #print(offload_fp32_gradients[i].dtype)
                #assert(offload_fp32_gradients[i].dtype == torch.float16)
    ...
    def _prepare_sub_group(self, sub_group_id, timer_names=set()):
        ...
        if self._swappable_optimizer_subgroup(sub_group_id):
            self._optimizer_states_and_gradient_swap_in(sub_group_id, timer_names, use_fpga = self.use_fpga)
    ...
    def _optimizer_states_and_gradient_swap_in(self, sub_group_id, timer_names=set(), use_fpga = False):
        ...
        #print("[swap-in is problem in ",dist.get_rank()," ]")
    ...
    def _release_sub_group(self, sub_group_id, timer_names=set()):
        ...
        if self._swappable_optimizer_subgroup(sub_group_id):
            self._optimizer_states_and_gradient_swap_out(sub_group_id, timer_names, use_fpga = self.use_fpga)
    ...
    def _optimizer_states_and_gradient_swap_out(self, sub_group_id, timer_names=set(), use_fpga = False):
        param_length = self.fp16_partitioned_groups_flat_numel[sub_group_id]
        fp32_param_id = id(self.fp32_partitioned_groups_flat[sub_group_id])
        assert self._swappable_optimizer_subgroup(sub_group_id), \
            f'Parameter {fp32_param_id} of numel={param_length} is not swappable'
    ...
    def step(self, closure=None):
        ...
        """
        #print("[pre_step is problem in ",dist.get_rank()," ]")
        self._pre_step()
        #print("[partition_all is problem in ",dist.get_rank()," ]")
        self._partition_all_parameters()

        #print("[overflow is problem in ",dist.get_rank()," ]")
        #checks for overflow, adjust the loss scale accordingly
        #if self._overflow_check_and_loss_scale_update():
        #    self._post_step(set())
        #    return

        #print("[get_global norm is problem in ",dist.get_rank()," ]")
        #norm_groups = self._get_norm_groups()
        #scaled_global_grad_norm = get_global_norm(norm_list=norm_groups)
        scaled_global_grad_norm = 1
        ...
        if self.use_fpga:
            print("# smart SSD: ", self.num_ssds)

            #get scale value the fp32 gradients
            ...
            combined_unscale = 1. / combined_scale
            #update parameters one sub group at a time
            

            process_groups = []
            total_processed_sub_group = 0
            
            mp_count = 1
            
            largest_numel = 0
            for s_id, _ in enumerate(self.fp16_groups):
                fp32_param = self.fp32_partitioned_groups_flat[s_id]
                swap_info = self.optimizer_swapper.swap_params_info.get(id(fp32_param), None)
                if largest_numel < swap_info.numel():
                    largest_numel = swap_info.numel()


            for sub_group_id, group in enumerate(self.fp16_groups):
                if (sub_group_id % self.num_ssds) == 0 :
                    
                    do_sync = ( ((sub_group_id / self.num_ssds) + 1) %  mp_count ==  0 )

                    sub_group_chunk_size = self.num_ssds 
                    if (sub_group_id + self.num_ssds ) > len(self.fp16_groups) :
                        sub_group_chunk_size =  len(self.fp16_groups) - sub_group_id
                        do_sync = True

                    total_processed_sub_group += sub_group_chunk_size

                    for s_id in range( sub_group_id, sub_group_id + sub_group_chunk_size ):
                        self._prepare_sub_group(s_id, timer_names)
                    
                    self._multiple_optimizer_step_with_fpga(sub_group_id, sub_group_chunk_size, combined_unscale, largest_numel )
                    
                    if do_sync:
                        print( "Progress <", sub_group_id, "/", len(self.fp16_groups), ">...")
                        for s_id in range( sub_group_id - sub_group_chunk_size * (mp_count - 1), sub_group_id + sub_group_chunk_size ):
                            self._release_sub_group(s_id, timer_names)
                            self.optimizer.sync_thread()
                            self._reassign_or_swap_out_partitioned_parameters(s_id)
                else:
                    continue
            assert( total_processed_sub_group == len(self.fp16_groups) )


        else:
            r = dist.get_rank()
            #print( "[",dist.get_rank(), "] Step starts...")
            #update parameters one sub group at a time
            for sub_group_id, group in enumerate(self.fp16_groups):

                #print("HERE [",len(self.fp16_groups), "/", sub_group_id + 1," in ", dist.get_rank(),"]")
                #prepare optimizer states, gradients and fp32 parameters for update
                #print("[Prepare is problem in ",r," ]")
                ...
                #print("optimization start at", dist.get_rank())
                #print("[Steop is problem in ",r," ]")
                self._optimizer_step(sub_group_id)
                ##print("optimization done at", dist.get_rank())
                #put fp16 parameters in appropriate location
                #print("[Reassign is problem in ",r," ]")
                self._reassign_or_swap_out_partitioned_parameters(sub_group_id)
                #print("After fp32 param : ", self.fp32_partitioned_groups_flat[sub_group_id].data[-10:])

                #release memory or swap out optimizer states of fp32 parameters
                #print("[Release is problem in ", r, " ]")
                self._release_sub_group(sub_group_id, timer_names)
                #print("[Success in ", r , " ]")

        #print( "[",dist.get_rank(), "] Step ends...")
        self.stop_timers(['optimizer_step'])

        self._post_step(timer_names)
        #print( "[",dist.get_rank(), "] Post Step ends...")
    ...
    def unscale_and_clip_grads(self, sub_group_id, total_norm):
        ...
        if self.clip_grad > 0.:
            ...
            if clip > 1:
                combined_scale = clip * self.loss_scale
        #print("Before gradient: ", self.fp32_partitioned_groups_flat[sub_group_id].grad[:10])
    ...
    def get_full_hp_param(self, param, optim_state_key=None) -> Tensor:
        ...
        if self._swappable_optimizer_subgroup(group_idx):
            self._optimizer_states_and_gradient_swap_in(group_idx, use_fpga = False)
        ...
        if self._swappable_optimizer_subgroup(group_idx):
            self._optimizer_states_and_gradient_swap_out(group_idx, use_fpga = False)
        return hp_param
