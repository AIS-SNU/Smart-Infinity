...
static const float scale = 0.003;
...
int adam_fpga(size_t nbytes, cl::Context context, cl::CommandQueue q, cl::Kernel krnl_adam){
	int err;
	...
	cl::Buffer grad(context, CL_MEM_READ_ONLY | CL_MEM_EXT_PTR_XILINX, nbytes, &outExt, nullptr);
	...
	float* p2p_param = (float*)q.enqueueMapBuffer(param,						// buffer
									  ...
									  nullptr, nullptr,
									  &err);
	float* p2p_grad = (float*)q.enqueueMapBuffer(grad,						// buffer
									  CL_TRUE,						// blocking call
									  CL_MAP_WRITE | CL_MAP_READ,	//Indicates we will write
									  0,							// buffer offset
									  nbytes,			// size in bytes
	...
	ret = pread(nvmeFd_grad, (void*)p2p_grad, nbytes, 0);
	if (ret == -1) {
		std::cout << "P2P: read() failed, err: " << ret << ", line: " << __LINE__ << std::endl;
		return EXIT_FAILURE;
	}
	...
	//q.enqueueReadBuffer(param16, CL_FALSE, offset, nbytes/2, &param16[0], &events, nullptr);
    q.finish();
...
int main(int argc, char* argv[]) {
	...
	cl_int err;
	const char* xclbinFilename = "./adam.xclbin";
	...
	ret = pwrite(nvmeFd,  &grad_src[0], nbytes, 0);
	if (ret == -1) {
		std::cout << strerror(errno) << std::endl;
		std::cout << "normal pwrite() failed, err: " << ret << ", line: " << __LINE__ << std::endl;
		return EXIT_FAILURE;
	...
	ret = adam_fpga(nbytes, context, q, krnl_adam);
	...
	//Verify the result
    int match = 0;
	AdamCPU( &param_ref[0], &grad_src[0], &exp_avg_ref[0], &exp_avg_sq_ref[0], DATA_SIZE );
