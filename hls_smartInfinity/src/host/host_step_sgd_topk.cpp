...
int sgd_fpga( cl::Context context, cl::CommandQueue q, cl::Kernel krnl_sgd, int padded_comp_grad_size, int padded_size){
	int err;
	...
	unsigned int cnt = 0;
	OCL_CHECK(err, err = krnl_sgd.setArg(cnt++, grad_idx));
    OCL_CHECK(err, err = krnl_sgd.setArg(cnt++, grad_val));
	krnl_sgd.setArg(cnt++, int(padded_comp_grad_size));

	OCL_CHECK(err, err = krnl_sgd.setArg(cnt++, grad));
	OCL_CHECK(err, err = krnl_sgd.setArg(cnt++, param16));
	OCL_CHECK(err, err = krnl_sgd.setArg(cnt++, param));
    OCL_CHECK(err, err = krnl_sgd.setArg(cnt++, exp_avg));
	krnl_sgd.setArg(cnt++, int(padded_size));
	...
	krnl_sgd.setArg(cnt++, _betta);
	krnl_sgd.setArg(cnt++, w_decay);
	krnl_sgd.setArg(cnt++, step_size);
	krnl_sgd.setArg(cnt++, scale);

    q.finish();
	std::chrono::high_resolution_clock::time_point compute_start = std::chrono::high_resolution_clock::now();
    //Launch the Kernel
	cl::Event run_event;	
    q.enqueueTask(krnl_sgd, &events, &run_event);
	...
	gbpersec = ((2 * nbytes + 2* comp_nbytes)/ dsduration) / ((double)1024 * 1024 * 1024);
	std::cout << "Compute : " << " Throughput = " << std::setprecision(2)
			  << std::fixed << gbpersec << "GB/s\n";
...
int main(int argc, char* argv[]) {
	...
	cl_int err;
	const char* xclbinFilename = "topk_sgd.xclbin";
	...
	if (ret == EXIT_FAILURE){
		std::cout<< "FPGA sgd failed ... " << std::endl;
		return EXIT_FAILURE;
	}
	...
	ret = sgd_fpga(context, q, krnl_sgd, padded_comp_grad_size, padded_size);
	std::cout << "FPGA sgd done!" << std::endl;
	...
	//Verify the result
    int match = 0;
	std::cout << "CPU sgd start..." << std::endl;
	sgd_CPU( &param_ref[0], &grad_ref[0], &grad_idx[0], &grad_val[0],  &exp_avg_ref[0], padded_size, padded_comp_grad_size);
	std::cout << "CPU sgd done!" << std::endl;
