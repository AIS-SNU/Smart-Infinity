...
int adagrad_fpga(size_t nbytes, cl::Context context, cl::CommandQueue q, cl::Kernel krnl_adagrad){
	int err;
	...
	unsigned int cnt = 0;
    OCL_CHECK(err, err = krnl_adagrad.setArg(cnt++, grad));
	OCL_CHECK(err, err = krnl_adagrad.setArg(cnt++, param16));
	OCL_CHECK(err, err = krnl_adagrad.setArg(cnt++, param));
    OCL_CHECK(err, err = krnl_adagrad.setArg(cnt++, exp_avg_sq));
	OCL_CHECK(err, err = krnl_adagrad.setArg(cnt++, DATA_SIZE));
	
	OCL_CHECK(err, err = krnl_adagrad.setArg(cnt++, _eps));
	OCL_CHECK(err, err = krnl_adagrad.setArg(cnt++, _weight_decay));
	OCL_CHECK(err, err = krnl_adagrad.setArg(cnt++, step_size));
	OCL_CHECK(err, err = krnl_adagrad.setArg(cnt++, scale));
    ...
    //Launch the Kernel
	cl::Event run_event;	
    q.enqueueTask(krnl_adagrad, &events, &run_event);
	...
	std::cout << "Compute : Buffer = " << 4 * nbytes << " Throughput = " << std::setprecision(2)
			  << std::fixed << gbpersec << "GB/s\n";
...
int main(int argc, char* argv[]) {
	...
	cl_int err;
	const char* xclbinFilename = "adagrad.xclbin";
    ...
    cl::Kernel krnl_adagrad(program,"krnl_vadd");
	delete[] buf;
	...
	ret = adagrad_fpga(nbytes, context, q, krnl_adagrad);

	if (ret == EXIT_FAILURE){
		std::cout<< "FPGA adagrad failed ... " << std::endl;
		return EXIT_FAILURE;
	}
	...
	//Verify the result
    int match = 0;
	AdagradCPU( &param_ref[0], &grad_src[0], &exp_avg_sq_ref[0], DATA_SIZE );
