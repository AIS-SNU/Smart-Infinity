...
int adagrad_fpga( cl::Context context, cl::CommandQueue q, cl::Kernel krnl_adam, int padded_comp_grad_size, int padded_size){
	int err;
	...
	*/
	...
	krnl_adam.setArg(cnt++, _weight_decay);
...
int main(int argc, char* argv[]) {
	...
	cl_int err;
	const char* xclbinFilename = "topk_adagrad.xclbin";
	...
	ret = adagrad_fpga(context, q, krnl_adagrad, padded_comp_grad_size, padded_size);
	if (ret == EXIT_FAILURE){
		std::cout<< "FPGA adagrad failed ... " << std::endl;
		return EXIT_FAILURE;
	}
	std::cout << "FPGA adagrad done!" << std::endl;
	...
	//Verify the result
    int match = 0;
	std::cout << "CPU adagrad start..." << std::endl;
	AdagradCPU( &param_ref[0], &grad_ref[0], &grad_idx[0], &grad_val[0],  &exp_avg_sq_ref[0], padded_size, padded_comp_grad_size);
	std::cout << "CPU adagrad done!" << std::endl;
